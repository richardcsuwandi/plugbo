...
def _lenz_create_args(
    ...
    args = ["--acqf", acqf, "--surrogate", surrogate, "--budget", str(budget)]
    if seed is not None:
        args += ["--seed", str(seed)]
    if constraints:
        args += ["--constraints", json.dumps(constraints)]
    if llm_provider and llm_model:
        args += ["--llm-provider", llm_provider, "--llm-model", llm_model]
        if llm_base_url:
            args += ["--llm-base-url", llm_base_url]
        if llm_api_key_env:
            args += ["--llm-api-key-env", llm_api_key_env]
        if llm_extra_body:
            args += ["--llm-extra-body", llm_extra_body]
...
def run_blind_test(
    ...
    if no_lenz:
        ...
        # Opt-in only: `warmup` defaults to None/0 for this branch (unlike
        # the lenz branch's seed-based default below), so an existing
        # --no-lenz invocation that never passes --warmup keeps its
        # historical evaluation-#1-is-the-agent's-own-choice behavior.
        # Passing --warmup N explicitly runs the *same* real Sobol
        # warm-start lenz-backed sibling conditions get -- generated by the
        # harness itself, never exposed to the agent (block_lenz only gates
        # the agent's own bash tool, not this call) -- so a comparison
        # against e.g. sara-lenz's N-point warm-start is apples to apples
        # instead of giving sara-only a free extra N evaluations.

...
def run_blind_test(
    ...
    if no_lenz:
        ...
        # Resolve the shared initial design exactly as the lenz-backed
        # branch does. The harness generates these points before lenz is
        # hidden from the agent, so every backend spends the same budget
        # and has the same regret trace through the warm-start.

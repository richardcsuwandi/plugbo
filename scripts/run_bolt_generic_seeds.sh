#!/usr/bin/env bash
# Extra seeds for BoLT LoRA's generic context -- brings it up to the same
# 3-seed (42, 43, 44) coverage as domain/misleading, so it clears the merged
# viewer's "2+ seeds" bar (results/logs/bolt_lora-generic-compare currently
# has only seed 42 for every condition).
#
#   ./scripts/run_bolt_generic_seeds.sh
#   ./scripts/run_bolt_generic_seeds.sh --list
#   ./scripts/run_bolt_generic_seeds.sh --status
#
# Same 6 conditions the existing generic seed-42 data has: vanilla, turbo
# (free, 2-wide), pibo (scripted, no agent), cake, sara-lenz, sara-only (LLM,
# one at a time). No sara-lenz-cake or sara-lenz-pibo here -- generic never
# had those either; this only extends what already exists to more seeds.
# Meant to run in its own terminal alongside
# ./scripts/run_ackley20_seeds.sh (independent benchmark groups, safe to run
# in parallel).
#
...
run_backend() {
  ...
  local log="$LOGDIR/bolt_generic_${backend}_seed${seed}.log"
  if [ "$backend" = "pibo" ]; then
    launch "$log" ./scripts/run_plugins.sh bolt_lora pibo "$seed" \
      --budget "$BUDGET" --context generic --group "$ROOT" \
      "${FORCE_FLAG[@]+"${FORCE_FLAG[@]}"}" "${LIST_FLAG[@]+"${LIST_FLAG[@]}"}"
    return 0
  fi
  launch "$log" ./scripts/run_bolt.sh \
    --context generic --backend "$backend" --seed "$seed" --budget "$BUDGET" \
    --root "$ROOT" \
    "${FORCE_FLAG[@]+"${FORCE_FLAG[@]}"}" "${LIST_FLAG[@]+"${LIST_FLAG[@]}"}"
}

echo "bolt_lora generic seeds=$SEEDS budget=$BUDGET root=$ROOT logdir=$LOGDIR"
...
echo "done. ./scripts/run_bolt_generic_seeds.sh --status"
echo "logs: $LOGDIR"
